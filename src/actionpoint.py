...
class ActionPoint:
            ...
            if tags is not False:
                ...
                if str(tags['ActionPoint']).lower() == "True".lower():
                    if (str(tags['ActionPointStart']).lower() != "False".lower()):
                        cronline=" root " + python + " " + command + " --cloud azure --action start --id " + self.azure.getVmId(vm['vmname'],vm['rgname'])
                        job = cron.new(command=cronline)
                        job.setall(tags['ActionPointStart'])
                        # job.user="root"
                    if (str(tags['ActionPointStop']).lower() != "False".lower()):
                        cronline=" root " + python + " " + command + " --cloud azure --action stop --id " + self.azure.getVmId(vm['vmname'],vm['rgname'])
                        job = cron.new(command=cronline)
    ...
    def testShhIsAlive(self,user,vmname,rgname,counter):
        ...
        if vmip is not False:
            while True:
                client.connect(hostname=vmip, username=user, pkey=key, auth_timeout=10, timeout=20)
                commands = [ "uptime" ]
                for command in commands:
            ...
            if args.action in action_available:
                    ...
                    if (str(tags['ActionPoint']).lower() == "True".lower() and str(
                        ...
                        if str(tags['ActionPointCustom']).lower() != "False".lower():
                            ...
                            if ap.startAzure(vmdata['vmname'], vmdata['resourcegroup']):
                                ...
                                if ap.runCustom(tags['ActionPointCustom'],tags['ActionPointUser'],vmdata['vmname'], vmdata['resourcegroup'],"start"):
                                    # print("Custom script executed with success")
                                    ap.log(vmdata['resourcegroup'] + "/" + vmdata['vmname'] + " Custom script executed with success.")
                                else:
                ...
                if (args.cloud == "azure" and args.action == "stop"):
                    ...
                    if (str(tags['ActionPoint']).lower() == "True".lower() and str(
                        ...
                        if str(tags['ActionPointCustom']).lower() != "False".lower():
                            # Run custom script and wait for return
                            if ap.runCustom(tags['ActionPointCustom'],tags['ActionPointUser'],vmdata['vmname'], vmdata['resourcegroup'],"stop"):
                                # print("Custom script executed with success")
                                ap.log(vmdata['resourcegroup'] + "/" + vmdata['vmname'] + " Custom script executed with success.")
                ...
                ap.log(vmdata['resourcegroup'] + "/" + vmdata['vmname'] + " Action \"" + args.action + "\" is not available.")
                exit(1)
        else:
